import { LoaderCircle, Pin, PinOff, Plus, Puzzle, SquareArrowOutUpRight, X } from "lucide-react";
  ...
  return (
    <div className="mx-auto w-full max-w-[880px] px-6 pt-7 pb-16">
      <header className="flex items-end justify-between gap-4 pb-4">
        ...
        <Button variant="ghost" size="sm" disabled={pendingAction !== null} className="text-primary hover:bg-soft hover:text-primary" onClick={install}>
          <Plus className="size-4" />添加扩展
        </Button>
      </header>

      {extensions === null ? loadError ? (
        <div role="alert" className="grid place-items-center gap-3 rounded-lg border border-dashed border-destructive-border py-20 text-center">
          <p className="text-sm text-destructive">扩展列表加载失败</p>
          <Button variant="secondary" size="sm" onClick={load}>重试</Button>
        </div>
      ) : (
        <div className="grid place-items-center rounded-lg border border-dashed border-border py-20 text-sm text-muted-foreground" aria-live="polite">
        ...
        <div className="grid place-items-center gap-3 rounded-lg border border-dashed border-border py-20 text-center">
          ...
          <Button variant="secondary" size="sm" disabled={pendingAction !== null} onClick={install}>
            <Plus className="size-4" />添加扩展
          </Button>
        </div>
      ) : (
          ...
          {extensions.map((extension) => (
            ...
            >
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogTitle>卸载「{extension.name}」？</AlertDialogTitle>
                  <AlertDialogDescription>将移除该扩展及其本地文件，此操作不可撤销。</AlertDialogDescription>
                  <AlertDialogFooter>
