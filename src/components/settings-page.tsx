...
function SettingsPage({
  ...
  const [lockOnSystemLock, setLockOnSystemLock] = useState(
    settings.lockOnSystemLock,
  );
  const [autoLockAfterHideSeconds, setAutoLockAfterHideSeconds] = useState(
    settings.autoLockAfterHideSeconds ?? 0,
  );
  ...
  const captureShortcut = (
    ...
    setter: (value: string) => void = setShortcut,
  ) => {
    ...
    <div className="pt-7 pb-0">
      <div className="mx-auto w-full max-w-[820px] px-6">
        ...
        <div className="flex flex-col gap-4">
          <SettingsGroup
            title="外观"
            description="跟随系统明暗，也可以固定为亮色或暗色，仅作用于 QuickPane 界面。"
          >
            <FieldRow label="主题">
              <Select
                value={theme}
          ...
          <SettingsGroup
            title="老板键"
            description="组合键仅在 QuickPane 运行或驻留托盘时有效。"
          >
          ...
          <SettingsGroup
            title="浏览"
            description="地址栏会识别网址，其余内容使用搜索引擎查询。"
          >
            <Field label="主页">
              <Input
                value={homeUrl}
          ...
          <SettingsGroup
            title="快捷站点"
            description="最多 12 个，显示在新标签页。"
          >
            <div className="flex flex-col gap-2">
              {quickLinks.map((link, index) => (
                <div
                  className="grid grid-cols-1 gap-2 sm:grid-cols-[130px_minmax(0,1fr)_auto]"
                  key={link.id}
                >
                  <Input
                    aria-label="站点名称"
          ...
          <SettingsGroup
            title="应用锁"
            description="应用密码只保护界面入口，不加密整个 WebView2 数据目录。"
          >
            <ToggleRow
              id="system-lock"
            ...
            <FieldRow label="隐藏后自动锁定">
              <Select
                ...
                onValueChange={(value) =>
                  setAutoLockAfterHideSeconds(Number(value))
                }
                disabled={!hasPassword}
              >
                <SelectTrigger
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() =>
                  void run(() =>
                    api.setPassword(password, currentPassword || undefined),
                  ).then((next) => {
                    if (next) {
                      applySnapshot(next);
                      setPassword("");
              ...
              {hasPassword ? (
                <Button
                  ...
                  onClick={() =>
                    void run(() => api.disablePassword(currentPassword)).then(
                      (next) => {
          ...
          <SettingsGroup
            title="软件更新"
            description="从配置的更新源检查并安装 QuickPane 新版本。"
          >
            ...
            {updateProgress ? (
              <div className="flex flex-col gap-1.5">
                ...
                <div
                  ...
                  aria-valuenow={
                    updateProgress.total ? updateProgress.downloaded : undefined
                  }
                  aria-valuetext={
                    updateProgress.total
                      ? `${Math.round((updateProgress.downloaded / updateProgress.total) * 100)}%`
                ...
                >
                  <div
                    className="relative h-full overflow-hidden rounded-full bg-gradient-to-r from-primary to-accent2 transition-[width]"
                    style={{
                  ...
                  >
                    <span
                      aria-hidden
                      className="absolute inset-0 animate-shimmer bg-gradient-to-r from-transparent via-white/25 to-transparent dark:via-white/15"
                    />
                  </div>
                </div>
              </div>
          ...
          <SettingsGroup
            title="浏览数据"
            description="清除站点数据会退出所有网站，并删除 WebView2 缓存。"
          >
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button
                  variant="secondary"
                  className="self-start border-destructive-border text-destructive hover:bg-destructive-soft hover:text-destructive"
                >
                  清除 Cookie 和站点数据
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                ...
                <AlertDialogDescription>
                  将退出所有已登录的网站并删除 WebView2 缓存，此操作不可撤销。
                </AlertDialogDescription>
                <AlertDialogFooter>
                  <AlertDialogCancel>取消</AlertDialogCancel>
                  <AlertDialogAction
                    variant="destructive"
                    onClick={() => void run(api.clearSiteData)}
                  >
                    清除
                  </AlertDialogAction>
                </AlertDialogFooter>
      ...
      <div className="sticky bottom-0 mt-6 flex items-center justify-end gap-3 border-t border-border bg-background/85 px-6 py-3 backdrop-blur">
        {saveState === "saved" ? (
          <span className="text-xs text-success">设置已保存</span>
        ) : null}
        {saveState === "error" ? (
  ...
  return (
    ...
    >
      <div className="min-w-0">
        <p id={labelId} className="text-xs font-medium">
          {label}
        </p>
