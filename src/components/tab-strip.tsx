...
function TabStrip({
  ...
  const openTabMenu = useCallback((tabId: string, event: React.MouseEvent) => {
    event.preventDefault();
    void api.showTabMenuWindow(tabId, event.clientX, event.clientY);
  }, []);

  return (
    <div className="relative z-30 flex h-8 shrink-0 select-none items-end bg-chrome pr-2 pl-2">
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        ...
        <div
          aria-hidden
          className="mt-1.5 ml-1 grid size-[18px] place-items-center rounded-sm bg-primary text-[10px] font-bold text-primary-foreground"
        >
          Q
        </div>
      </div>
      ...
      {panel ? (
        <div className="absolute top-[calc(100%+4px)] right-2 w-[min(360px,calc(100vw-16px))] overflow-hidden rounded-md border bg-popover p-1.5 text-popover-foreground shadow-popover">
          {panel === "search" ? (
            <>
