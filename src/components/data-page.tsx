...
/** 页面容器 + 标题 */
function PageShell({ title, count, clearLabel, onClear, children }: {
  title: string;
  count: number;
  clearLabel?: string;
  onClear?: () => void;
    ...
    <div className="mx-auto w-full max-w-[880px] px-6 pt-7 pb-16">
      <header className="flex items-end justify-between gap-4 pb-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">{count} 项</p>
        </div>
        {onClear && count > 0 ? (
          <AlertDialog>
            <AlertDialogTrigger asChild>
              ...
              >
                <Trash2 className="size-4" />{clearLabel ?? "清除记录"}
              </Button>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogTitle>清除全部{title}？</AlertDialogTitle>
              <AlertDialogDescription>将删除 {count} 项记录，此操作不可撤销。</AlertDialogDescription>
              <AlertDialogFooter>
                <AlertDialogCancel>取消</AlertDialogCancel>
                <AlertDialogAction variant="destructive" onClick={onClear}>清除</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        ) : null}
...
function DataRow({ icon, title, subtitle, meta, url, onOpen, onRemove, removeLabel }: {
  icon: ReactNode;
  title: string;
  subtitle: string;
  meta: string;
...
function HistoryPage({ snapshot, onOpen, onClear }: { snapshot: AppSnapshot; onOpen: (url: string) => void; onClear: () => void }) {
  return (
    <PageShell title="历史记录" count={snapshot.data.history.length} onClear={onClear}>
      {snapshot.data.history.length === 0 ? (
        <EmptyState icon={<History />} title="还没有访问记录" hint="通过地址栏浏览的网页会出现在这里，点击即可重新打开。" />
      ) : (
...
function BookmarksPage({ snapshot, onOpen, onRemove }: { snapshot: AppSnapshot; onOpen: (url: string) => void; onRemove: (id: string) => void }) {
  return (
    <PageShell title="书签" count={snapshot.data.bookmarks.length}>
      {snapshot.data.bookmarks.length === 0 ? (
        <EmptyState icon={<Star />} title="收藏的网页会显示在这里" hint="浏览时点击地址栏右侧的星标即可收藏当前页面。" />
      ) : (
        snapshot.data.bookmarks.map((item) => (
          <DataRow
            key={item.id}
            icon={<Star />}
            title={item.title || hostOf(item.url)}
...
function DownloadsPage({ snapshot, onOpen, onClear }: { snapshot: AppSnapshot; onOpen: (path: string) => void; onClear: () => void }) {
  return (
    <PageShell title="下载" count={snapshot.data.downloads.length} onClear={onClear}>
      {snapshot.data.downloads.length === 0 ? (
        <EmptyState icon={<Download />} title="下载记录会显示在这里" hint="完成的下载可以直接从列表中打开。" />
      ) : (
        snapshot.data.downloads.map((item) => (
          <DataRow
            ...
            meta={item.state === "completed" ? "已完成" : item.state === "failed" ? "失败" : "下载中"}
            onOpen={item.path ? () => onOpen(item.path!) : undefined}
          />
        ))
      )}
    </PageShell>
